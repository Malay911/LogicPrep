...
SELECT customer.first_name,customer.last_name,color.name
FROM customer JOIN color
ON customer.id=color.id
...
FROM clothing_order JOIN clothing
ON clothing_order.customer_id=clothing.id
JOIN customer
ON clothing.id=customer.id
JOIN category
ON category.id=clothing.category_id
WHERE customer.first_name='Jay' AND category.name='T-Shirt' AND clothing_order.order_date>'2024-04-01'
...
SELECT category.name,COUNT(clothing.id) as num_items,MAX(clothing.price) as max_price,MIN(clothing.price) as min_price,AVG(clothing.price) as avg_price
...
SELECT CONCAT(customer.first_name,' ',customer.last_name) as customer_name,COUNT(clothing_order.id) as num_orders,SUM(clothing_order.items) as total_items,SUM(clothing.price) as total_purchase_value
...
SELECT clothing.name,clothing.size,COUNT(clothing_order.items) as items_sold,SUM(clothing.price) as order_value
...
ON customer.id=clothing_order.customer_id
JOIN clothing
ON clothing_order.clothing_id=clothing.id
