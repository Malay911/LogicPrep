...
--🚀 Paper 3: SQL Queries
--1: List customers who have never placed any order.
SELECT Customers.CustomerID,Customers.JoinDate,Customers.Name
FROM Customers LEFT JOIN Orders 
ON Customers.CustomerID=Orders.CustomerID
WHERE Orders.OrderID IS NULL

--2: List products ordered by more than 5 different customers.
SELECT Products.ProductName,COUNT(Customers.CustomerID) as DifferentCustomerCount
...
--3: List customers whose first order was placed in 2024.
SELECT Customers.Name,MIN(Orders.OrderDate)
FROM Customers JOIN Orders
ON Customers.CustomerID=Orders.CustomerID
WHERE YEAR(Orders.OrderDate)='2024'
GROUP BY Customers.Name

--4: List top 3 products with the highest total revenue.
SELECT TOP 3 Products.ProductName,SUM(Orders.Quantity*Products.Price) as TotalRevenue
FROM Products JOIN Orders
ON Products.ProductID=Orders.ProductID
GROUP BY Products.ProductName
ORDER BY SUM(Orders.Quantity*Products.Price) DESC

--5: Find the latest order placed for each customer.
SELECT Customers.Name,MAX(Orders.OrderDate) as LatestOrderDate
FROM Customers JOIN Orders
ON Customers.CustomerID=Orders.CustomerID
GROUP BY Customers.Name

--6: List products that have been ordered but are currently out of stock.
SELECT Products.ProductName,Products.StockQuantity
FROM Products JOIN Orders
ON Products.ProductID=Orders.ProductID
WHERE Products.StockQuantity=0

--7: List customers who placed orders in more than one product category.
SELECT Customers.Name,COUNT(DISTINCT Products.Category) as UniqueCategoriesCount
FROM Customers JOIN Orders
ON Customers.CustomerID=Orders.CustomerID
JOIN Products
...
--8: List products with no orders in the last 6 months.
SELECT Products.ProductName
FROM Products LEFT JOIN Orders
ON Products.ProductID=Orders.ProductID
AND Orders.OrderDate>=DATEADD(MONTH,-6,GETDATE())
...
--10: Find the product whose price is second highest.
